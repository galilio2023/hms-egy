...
export default async function AdmissionsPage({
  ...
  const bedsWithAdmissions = await db
    .select({
      bedId: beds.id,
  ...
  const [housekeepingRes] = await db
  ...
  return (
    <div className="min-h-screen bg-gray-50/10 py-8 px-4 sm:px-6 lg:px-8">
      <AdmissionsDashboardClient
        ...
        rooms={roomsList}
        bedsData={bedsWithAdmissions}
        doctors={doctorsList}
        vitalsHistory={vitalsByPatient}
        pendingCleaningCount={pendingCleaningCount}
      />
    </div>
  );
}
